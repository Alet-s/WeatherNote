...
fun MunicipiosScreen(
    ...
    var idInput = remember { androidx.compose.runtime.mutableStateOf(TextFieldValue()) }
    ...
    Scaffold(
        ...
        Column(
            ...
            Row(
                ...
                Spacer(modifier = Modifier.width(8.dp))
                Button(onClick = {
                    if (idInput.value.text.isNotBlank() && nameInput.value.text.isNotBlank()) {
                        viewModel.addMunicipio(idInput.value.text, nameInput.value.text)
                        idInput.value = TextFieldValue()
